...
public class NetworkActivity extends CameraActivity implements OnImageAvailableListener {
  ...
  private ControlPublisherNode controlPublisherNode;
  ...
  protected void init(NodeMainExecutor nodeMainExecutor) {
    controlPublisherNode = new ControlPublisherNode();
    ...
    nodeMainExecutor.execute(controlPublisherNode, nodeConfiguration);
  }

  protected void sendRosControl(ControlSignal vehicleControl){
    controlPublisherNode.publish(vehicleControl);
  }
    ...
    if (noiseEnabled) {
      ...
      if (currentTime < noiseStartTime + noiseDuration) {
        ...
        if (noiseDirection < 0){
          ...
          sendRosControl(new ControlSignal(vehicleControl.getLeft() - noise, vehicleControl.getRight()));}
        else{
          sendControlToVehicle(new ControlSignal(vehicleControl.getLeft(), vehicleControl.getRight() - noise));
          sendRosControl(new ControlSignal(vehicleControl.getLeft() , vehicleControl.getRight()-noise));}

      } else {
        sendRosControl(vehicleControl);
        sendControlToVehicle(vehicleControl);
      }
    } else { // if noise is not enabled
      sendControlToVehicle(vehicleControl);
      sendRosControl(vehicleControl);
    }
  }
