...
public class MultiBoxTracker {
  ...
  public synchronized ControlSignal updateTarget() {
    if (!trackedObjects.isEmpty()) {
      ...
      // additional code for stopping
      float boxWidth = (rotated ?  trackedPos.height() : trackedPos.width());
      float boxScale = boxWidth/imgWidth;
      if (((boxScale > 0.45f) && (rotated))||((boxScale > 0.6f) && (!rotated))){
        if (((boxScale > 0.55f) && (rotated))||((boxScale > 0.8f) && (!rotated))){
          if (x_pos_scaled < 0.0f){
            leftControl = -1.0f;
            rightControl = -1.0f - x_pos_scaled;
          } else {
            leftControl = -1.0f + x_pos_scaled;
